  ...
  return (
    <>
      <div className="relative grow">
        ...
        <div className="px-28 py-12 text-16 font-600 text-brand-grey-600">
          나의 최애 아티스트
        </div>
        {selectedArtists.length !== 0 ? (
          <div className="max-h-400 overflow-y-auto">
            {selectedArtists.map((artist) => (
              ...
              >
                <span className="line-clamp-1 text-16 font-400 text-brand-grey-700">
                  {artist.artistName}
                </span>
                <button
                  type="button"
      ...
      {/* 수정모드 */}
      {isListOpen && (
        <div className="absolute -top-44 bottom-0 left-0 right-0 z-[51] flex flex-col bg-basic-white">
          <SearchInput
            value={searchValue}
          ...
          <div className="grow overflow-y-auto pt-12">
            {editingFilteredArtists.map((artist) => (
              ...
              >
                ...
                />
                <span className="text-16 font-400 text-brand-grey-700">
                  {artist.artistName}
                </span>
              </button>
            ))}
          </div>
          <div className="absolute bottom-0 left-0 right-0 bg-basic-white px-32 pb-16 pt-12">
            <Button
              type="button"
              variant="primary"
