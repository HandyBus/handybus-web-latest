...
import type { ReviewsType, ReviewType } from '@/types/review.types';

const REVIEW_PER_PAGE = 10;
const ANY_IMAGE =
  'https://d2mcfseiqyiloq.cloudfront.net/concerts/images/23ecc7e3-9f39-4a34-9204-2ecdc9949ffb.jpg';
...
const fakeReviews: ReviewType[] = MOCKING_REVIEWS.map((content, index) => ({
  user: {
    userId: index,
    nickname: `아이유사랑해${index}`,
    profileImageUrl: ANY_IMAGE,
  },
  concert: {
    concertId: index,
    location: '잠실종합체육관',
...
const getReviewsMocking = async (page: number) => {
  const deeeeepcopy = JSON.parse(JSON.stringify(fakeReviews)) as ReviewType[];
  return {
