    ...
    >
      ...
      <div className="flex h-[130px] w-full gap-16">
        ...
        <div className="flex flex-col">
          <span className="line-clamp-1 pb-4 text-16 font-500 text-grey-700">
          ...
          <span className="pt-4 text-14 font-500 text-grey-700">
...
const Button = ({ text, onClick, disabled, color }: ButtonProps) => {
  ...
  return (
    <button
      ...
      disabled={disabled}
      type="button"
      className={`flex h-40 w-full items-center justify-center rounded-full text-14 font-500 disabled:bg-grey-50 disabled:text-grey-300 ${color === 'primary' ? 'bg-primary-main text-white active:bg-primary-600' : 'bg-grey-50 text-grey-700 active:bg-grey-100'}`}
    >
      {text}
    </button>
  );
