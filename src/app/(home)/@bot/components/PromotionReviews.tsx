  ...
  return (
    ...
    >
      {top3.map((review) => (
        <div
          key={review.reviewId}
          className="mx-16 mt-8 flex flex-col gap-[10px] rounded-[16px] bg-brand-grey-50 p-16"
        >
          <Rating size="medium" value={review.rating} />
          <p className="line-clamp-2 overflow-hidden text-14 font-500 text-brand-grey-600">
            {review.content}
          </p>
        </div>
      ))}
