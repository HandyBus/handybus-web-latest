  ...
  return (
    ...
    >
      <div className="flex w-220 flex-col gap-8">
        <div className="relative h-280 w-220 overflow-hidden rounded-[12px] bg-brand-grey-600">
          <Image
        ...
        <div className="flex flex-col gap-4 pl-4">
          <span className="line-clamp-2 text-16 font-600 text-brand-grey-700">
            [{route.name}]{' '}
...
const SeatString = ({ route }: SeatStringProps) => {
  ...
  return (
    <div className="absolute bottom-0 w-full bg-basic-black bg-opacity-70 px-16 py-12 text-right text-14 font-500 text-basic-white backdrop-blur-[2px]">
      {isSoldOut ? (
        <span className="text-brand-grey-100">매진</span>
      ) : (
        <>
          {prefix}{' '}
